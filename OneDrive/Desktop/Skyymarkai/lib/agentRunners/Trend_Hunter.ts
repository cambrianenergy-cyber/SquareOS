...
export const TrendHunterRunner: AgentRunner = async (input: AgentRunnerInput): Promise<AgentRunnerOutput> => {
  ...
  try {
    ...
    const output = {
      trends: [
        { topic: "AI automation tools", platform: "LinkedIn", strength: "High", reason: "B2B tech adoption surge" },
        { topic: "Short-form video content", platform: "Instagram/TikTok", strength: "Very High", reason: "Algorithm favorability" },
        { topic: "Authentic brand storytelling", platform: "All platforms", strength: "High", reason: "Audience trust preference" },
        { topic: "Interactive polls/quizzes", platform: "Instagram Stories", strength: "Medium", reason: "Engagement boost" },
        { topic: "Behind-the-scenes content", platform: "TikTok", strength: "High", reason: "Humanizes brands" }
      ],
      recommendations: "Focus on short-form video and authentic storytelling for maximum engagement",
    ...
    if (runId) {
      await logAgentRunSuccess({
        ...
        workspaceId: input.workspaceId,
        outputs: {
          summary: `Generated trend recommendations`,
        },
        duration: Date.now() - startTime
      });
    }
    ...
    if (runId) {
      await logAgentRunFailure({
        ...
        workspaceId: input.workspaceId,
        error: {
          message: error.message || "Trend Hunter execution failed",
